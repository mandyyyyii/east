...
class CustomDataCollatorForLanguageModeling(DataCollatorForLanguageModeling):
    def __call__(self, features):
        # 1) Store the extra fields we need for training
        extra_fields = {
            "entropy": [],
            "chosen_weight": [],
            "rejected_weight": []
        ...
        for f in features:
            for field in extra_fields:
                if field in f:
                    extra_fields[field].append(f[field])

        # 2) Create filtered features with only the tokenizer-compatible fields
        filtered_features = []
        for f in features:
            filtered_feature = {
        ...
        # 3) Let parent class handle padding of the main fields
        batch = super().__call__(filtered_features)

        # 4) Add back the extra fields to the batch
        for field, values in extra_fields.items():
            if values:  # Only add if we collected any values
                batch[field] = torch.tensor(values, device=batch["input_ids"].device)
...
class SFTTrainer(Trainer):
    ...
    def __init__(
        ...
        if not args.packing:
            if data_collator is None:
                ...
                data_collator = CustomDataCollatorForLanguageModeling(tokenizer=processing_class, mlm=False)
    ...
    def _prepare_non_packed_dataloader(
        ...
        def tokenize(element):
            ...
            for i in range(len(element['prompt'])):
                # Concatenate corresponding prompt and chosen text
                full_texts.append(element['prompt'][i] + element['chosen'][i])
            ...
            for i in range(len(prompt_tokens['input_ids'])):
                ...
                # Create labels with -100 for prompt tokens and actual ids for response tokens
                sample_labels = [-100] * prompt_length + outputs['input_ids'][i][prompt_length:]
            ...
            return {
                ...
                "chosen_weight": element["chosen_weight"], 
                "rejected_weight": element["rejected_weight"]
            }

        signature_columns = ["input_ids", "labels", "attention_mask", "entropy", "chosen_weight", "rejected_weight"]
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        # 1) Get the base loss from Trainer
        original_loss, outputs = super().compute_loss(model, inputs, return_outputs=True)

        # 2) Grab weights
        entropy = inputs.get('entropy', None)
        chosen_weight = inputs.get('chosen_weight', None)
        rejected_weight = inputs.get('rejected_weight', None)
        ...
        def _calculate_rejected_weight_non_linear(self, rejected):
            """Calculate weight based on entropy values."""
            return rejected**self.use_rejected_weight_non_linear/self.use_rejected_weight_non_linear_coeff
        ...
        if self.use_weighting:
            entropy = entropy.clone().detach().to(original_loss.device)
            chosen_weight = chosen_weight.clone().detach().to(original_loss.device)
            rejected_weight = rejected_weight.clone().detach().to(original_loss.device)

            if self.use_chosen_weight_non_linear:
                weight = _calculate_chosen_weight_non_linear(self, chosen_weight)
                self.log({
                    "train/non_linear_weight_mean": weight.mean().item()
                })

            elif self.use_entropy_non_linear:
                weight = _calculate_entropy_weight_non_linear(self, entropy)
                self.log({
                        "train/non_linear_weight_mean": weight.mean().item()
                    })
            elif self.use_rejected_weight_non_linear:
                weight = _calculate_rejected_weight_non_linear(self, rejected_weight)
                self.log({
                        "train/non_linear_weight_mean": weight.mean().item()
                    })
            weighted_loss = (original_loss * weight).mean()

        else:
            weighted_loss = original_loss.mean()

        return (weighted_loss, outputs) if return_outputs else weighted_loss
    def log(self, metrics: Dict[str, float],start_time=None) -> None:
        """
        Log metrics using available logging mechanisms.
        """
